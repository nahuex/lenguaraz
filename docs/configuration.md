...
| `TRANSLATE_HEDGE_AFTER_MS` | integer 0–30000 | `1200` | Hedged requests: when a translation has not answered after this many milliseconds, an identical request is sent and the first answer wins (the others are cancelled). Cuts the long tail when the model queues some requests for 10–40 s. `0` disables hedging. |
| `TRANSLATE_HEDGES` | integer 0–4 | `2` | Maximum extra copies per translation (sent one `TRANSLATE_HEDGE_AFTER_MS` apart). Translations are short, so the extra cost is small; counted as `translation_hedged`. |
| `TRANSLATE_ORDER_WAIT_MS` | integer 0–30000 | `1500` | Head-of-line cap: a translated sentence waits at most this long for the previous one before it is published; a sentence that finishes after a later one was shown is dropped from that language (counted as untranslated), so the on-screen order never breaks and one slow call never freezes the view. |
