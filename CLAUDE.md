...
- **Frontend:** Vite + React + TS + Tailwind; pages Home `/` (stages + state + QR), **Fogón** `/fogon/{stage}` (captions, language, font size, contrast, dark), **Pizarrón** `/pizarron/{stage}` (transparent overlay, `?lang=&lines=2`), **Mangrullo** `/mangrullo` (Bearer). Reconnecting WebSocket with backoff.
- **Naming:** follow the constitution's Naming convention table — surface names (Oído, Lengua, Parla, Posta, Baqueano, Chasque, Fogón, Mangrullo, Pizarrón, Diccionario, Acta) in UI, routes, CLI, metrics and docs; technical module names in code.
...
- [ ] README (EN) + README.es.md: **the Lenguaraz story (one paragraph) + naming glossary table**, pitch, GIF, 3-command quickstart, architecture, stages.yaml guide, OBS/SRT guide, scaling to 30+ stages, cost per stage-hour (measured + pricing date), security & privacy (key server-side, ephemeral tokens, paid tier, 18+), limitations, how we built it (SDD + loop, link to constitution), license
