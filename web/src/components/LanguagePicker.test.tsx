...
describe('LanguagePicker', () => {
  ...
  it('renders a named radiogroup with one radio per language and marks the original', () => {
    ...
    expect(screen.getByRole('radio', { name: 'English (original audio)' })).toBeChecked();
    expect(screen.getByRole('radio', { name: 'Spanish (translation)' })).not.toBeChecked();
  });

  it('calls onChange with the short code', () => {
    const onChange = vi.fn();
    render(<LanguagePicker options={options} value="en" onChange={onChange} />);
    fireEvent.click(screen.getByRole('radio', { name: 'Spanish (translation)' }));
  ...
  it('selects a language from its label too', () => {
    const onChange = vi.fn();
    render(<LanguagePicker options={options} value="en" onChange={onChange} />);
    fireEvent.click(screen.getByText('Spanish (translation)'));
    expect(onChange).toHaveBeenCalledWith('es');
  });
});
