...
**Status:** Shipped (H1 OK 2026-09-24T22:40Z) · **Owner:** human · **Author:** agent · **Created:** 2026-09-24T15:58Z
...
| FR-001-13 | **Observability:** JSON logs carrying `stage_id`, `session_id`, `seq` and `component` (surface name); every caption event MUST carry `t_audio_ms` (audio-timeline position of the newest chunk sent) and `latency_ms`: for an interim, milliseconds since the previous partial update of the same utterance (0 for the first); for a final, milliseconds between the last partial update and the committed line (commit delay). Speech-to-caption latency (sentence end → final, sentence start → first partial) is measured by `make smoke-stt` against the samples' sentence boundaries and recorded in `docs/metrics.md`. A `metrics` event per stage every 5 s carries p50/p95 of the final commit delay over the last 200 captions, `rotations` and `errors`. Transcript text MUST NOT be logged unless `LOG_TRANSCRIPTS=true`. | Art. V.1 · Art. IX.2 |
...
|---|---|---|
| NFR-001-01 | Interim caption visible after speech | p95 ≤ 1.5 s from sentence start to the first partial caption, measured by `make smoke-stt` with the samples' sentence boundaries, recorded in `docs/metrics.md` with date |
...
- [x] Q4 Latency definition → amended, see changelog 2026-09-24T16:45Z

## Changelog
- 2026-09-24T15:58Z created; Q1–Q4 resolved with defaults per CLAUDE.md §5 (logged in HUMAN_INBOX.md); Status Approved.
- 2026-09-24T16:45Z **amended FR-001-13 / NFR-001-01** after the first real run: the original `latency_ms` (publish time minus audio-timeline position) is ≈ 0 whenever the source is real-time paced, so it measured ingest lag, not caption latency. New definition: interim = update gap, final = commit delay; true speech-to-caption latency is measured by `make smoke-stt` with known sentence boundaries (first run: sentence end → final p50/p95 = 531/760 ms).
