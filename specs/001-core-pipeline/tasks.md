...
| T-001-17 | **Hybrid VAD** (added 17:17Z after the first real runs, D-001-4): client-side silence detection in the sender (`VAD_MODE=hybrid`, `VAD_SILENCE_MS`, `VAD_THRESHOLD`) sends `audio_stream_end` after speech + silence so the server finalizes fast; fake session tolerates it; docs updated | FR-001-03, NFR-001-01, Art. V | `uv run pytest tests/test_stt_session.py -k vad` + `make smoke-stt` (utterance-to-final p95 < 2 s) | ☑ |
