...
- 2026-09-25T02:30Z T-012-01/02: shadcn CLI v4.21 names the default style `nova` (components.json `style: "radix-nova"`, Radix base, neutral, radius 0.5 rem) — this is what Q1's "new-york" resolves to; the CLI installs `radix-ui` (unified) and `cn` instead of `@radix-ui/react-*` and clsx + tailwind-merge (FR-012-04 allowlist unchanged: all MIT/ISC/Apache-2.0).
- 2026-09-25T02:45Z T-012-04/05: `vitest-axe` 0.1.0 (MIT; brings axe-core, MPL-2.0, dev-only) runs axe on StageCard, LanguagePicker, A11yControls, CaptionView, the Layout shell and the Admin sign-in form (`web/src/test/a11y.test.tsx`); the `color-contrast` rule is off in jsdom (no layout engine; it only ever returns "needs review"), contrast stays covered by the token pairs and H-UI. Legacy tokens (`--surface`, `--ink`, `--line`, `--accent-legacy`, `--interim` and their utilities) deleted from `index.css`; `--caption-size` and `--focus` kept. NFR-012-01 measured: main chunk 429.64 kB / **136.23 kB gzip** (≤ 250 kB). Docs: customization "Theming", CONTRIBUTING "UI components", CHANGELOG entry. AC-4 (H-UI) pending the owner.
